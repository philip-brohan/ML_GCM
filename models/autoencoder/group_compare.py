...
model_save_file = ("%s/ML_GCM/autoencoder/" + "Epoch_%04d/generator") % (
    os.getenv("SCRATCH"),
    args.epoch,
)
...
def three_plot(ax, t2m, u10m, v10m, prmsl):
    ...
    lsmask = iris.load_cube(
        "%s/fixed_fields/land_mask/opfc_global_2019.nc" % os.getenv("SCRATCH")
    )
...
def get_axis(n):
    ...
    if n % 2 == 1:
        xmin = w + offset * 2
    ymin = [offset, h + offset * 2, 2 * h + offset * 3, 3 * h + offset * 4][
        (n - 1) // 2
    ]
